function zoekButtonClicked() {
  ...
  const jaarVan = document.getElementById('jaarVan').value;
  const jaarTot = document.getElementById('jaarTot').value;
  ...
  if (amsterdamArchive.checked === true) {
    window.open(
      `https://archief.amsterdam/indexen/persons?sa=%7B%22person_1%22:%7B%22q%22:%22${keywords}%22,%22search_t_geslachtsnaam%22:%22${achternaam}%22,%22search_t_tussenvoegsel%22:%22${tussenvoegsel}%22,%22search_t_voornaam%22:%22${voornaam}%22%7D%7D`,
      '_blank',
    );
  }
}
...
//   .addEventListener('click', zoekButtonClicked());
